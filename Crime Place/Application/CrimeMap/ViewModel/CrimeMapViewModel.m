...
-(Boolean) checkIfCoordinateOne:(double)item1 isAlmostNearToCoordinate2:(double)item2{
 ...
 */
-(void) getPlacesFromLatitude:(double)latitude andLongitude:(double)longitude {
    [_delegate displayActivityIndicator:YES];
    CrimeMapViewModel * __weak weakSelf = self;
    [apiService getCrimePlacesfromLatitute:latitude andLongitude:longitude atDate:DEFAULT_DATE withCompletion:^(NSArray * _Nonnull places, NSString * _Nonnull error) {
        if(error){
...
-(GMSMarker *) createMarkerWithTitle:(NSString *)title andSnippet:(NSString *)snippet fromLat:(double) lat andLng:(double)lng{
